...
fn compute_def_use_sets(method_cfg: &CFG, x86_blocks: &HashMap<i32, Vec<X86Insn>>) -> HashMap<i32, DefUse> {
   ...
   for (block_id, _) in method_cfg.blocks.iter() {
       ...
       for insn in &x86_blocks[block_id] {
...
fn compute_maps(method_cfg: &CFG, x86_blocks: &HashMap<i32, Vec<X86Insn>>, debug: bool) -> (HashMap<i32, HashSet<X86Operand>>, HashMap<i32, HashSet<X86Operand>>) {
...
fn liveness_analysis(method_cfg: &CFG, x86_blocks: &HashMap<i32, Vec<X86Insn>>, debug: bool) -> HashMap<i32, Vec<HashSet<X86Operand>>> {
   let (in_map, out_map) = compute_maps(method_cfg, x86_blocks, debug);
...
pub fn peephole(method_cfg: &CFG, x86_blocks: &mut HashMap<i32, Vec<X86Insn>>) {
   // todo!()
}