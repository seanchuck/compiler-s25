...
fn map_operand(
    ...
    match operand {
        ...
        Operand::LocalArrElement(arr, idx) => {
            let array_typ: Type = method_cfg.locals.get(arr).expect("expected array entry").typ;
            ...
            x86_instructions.push(X86Insn::Mov(idx_op, X86Operand::Reg(Register::R10), array_typ)); // store index in r10
            x86_instructions.push(X86Insn::Add(X86Operand::Constant(1), X86Operand::Reg(Register::R10), array_typ)); // add one to index because first element is length
...
fn add_instruction(method_cfg: &CFG, insn: &Instruction, x86_instructions: &mut Vec<X86Insn>) {
    match insn {
        ...
        Instruction::Exit { exit_code } => {
            x86_instructions.push(X86Insn::Mov(X86Operand::Constant(*exit_code), X86Operand::Reg(Register::Rax)));
