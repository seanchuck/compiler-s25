...
use crate::scope::{EnclosingBlock, Scope, TableEntry};
use crate::symtable::*;
...
pub fn format_error_message0(
    ...
    match span {
        ...
        None => format!("~~~{}: semantic error: \n|\t{}", context.filename, msg),
    }
}

pub fn format_error_message2(
    invalid_token1: &str,
    invalid_token2: &str,
    span: Option<&Span>,
    msg: &str,
            ...
            for method in methods {
                match **method {
                    AST::MethodDecl {
                        ...
                        let method: SymMethod =
                            build_method(method, Rc::clone(&global_scope), writer, context);
    ...
    match method {
        AST::MethodDecl {
            ...
            let method_body = match **block {
                AST::Block { .. } => build_block(
                    block,
                    Rc::clone(&method_scope),
                    writer,
                    context,
                    Some(Rc::clone(&method_scope)),
                ),
                        ...
                        for decl in decls {
                            match **decl {
                                ...
                                AST::ArrayFieldDecl {
                                    ...
                                    check_and_insert_array_field(
                                        id, size, span, typ, &scope, writer, context,
                                    );
    ...
    match statement {
        ...
        AST::Statement(Statement::For {
            ...
            if let Some(var_entry) = scope.borrow().lookup(var) {
                match var_entry {
                    TableEntry::Variable { typ, span, .. } => match typ {
                        Type::Int | Type::Long => {
                            var_typ = typ;
                        }
                        _ => {
                            writeln!(
                                ...
                                format_error_message(
                                    var,
                                    Some(&span),
                                    "For loop variable must be int or long",
                                    context
                                )
                            )
                            .expect("Failed to write output!");
                    ...
                    _ => {
                        writeln!(
                            ...
                            format_error_message(
                                var,
                                Some(span),
                                "For loop variable must be a table entry variable",
                                context
                            )
                        )
                        .expect("Failed to write output!");
                ...
                writeln!(
                    ...
                    format_error_message(
                        var,
                        Some(span),
                        "For loop variable not defined in scope",
                        context
                    ) // TODO: Check if necessary to check here
                )
                .expect("Failed to write output!");
            }
...
pub fn build_expr(
    ...
    match expr {
            ...
            match (op, &**expr) {
                ...
                (UnaryOp::Neg, AST::Expr(Expr::Literal { lit, .. })) => {
                    return SymExpr::Literal {
                        value: match lit {
                            Literal::Int(value) => {
                                Literal::Int(("-".to_owned() + value).to_string())
                            }
                            Literal::Long(value) => {
                                Literal::Long(("-".to_owned() + value).to_string())
                            }
                            Literal::HexInt(value) => {
                                Literal::HexInt(("-".to_owned() + value).to_string())
                            }
                            Literal::HexLong(value) => {
                                Literal::HexLong(("-".to_owned() + value).to_string())
                            }
        ...
        AST::Expr(Expr::ArrAccess { id, index, span }) => SymExpr::ArrAccess {
            id: id.clone(),
        ...
        _ => panic!("Error in build_expr: unexpected AST node:\n {:#?}", expr),
    }
}
                ...
                if name == "main" {
                    ...
                    if *return_type != Type::Void {
                        ...
                        let error_msg = format_error_message(
                            "main",
                            Some(&span),
                            "should not have any parameters: entry point",
                            context,
                        );
                        writeln!(writer, "{}", error_msg).expect("Failed to write error message");
                    }
        ...
        if size_value <= 0 {
            writeln!(
                ...
                format_error_message(
                    id,
                    Some(span),
                    "array field declaration must have size > 0",
                    context
                )
            )
            .expect("Failed to write output!");
